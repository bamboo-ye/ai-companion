...
func TestMemoryArbitrationRoundTrip(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = store.db.ExecContext(ctx, "DELETE FROM long_term_memories WHERE user_id=UUID_TO_BIN(?)", userID)
		_, _ = store.db.ExecContext(ctx, "DELETE FROM users WHERE id=UUID_TO_BIN(?)", userID)
	})
