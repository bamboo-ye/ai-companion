...
The PDF translation tool is an independently persisted Skill Run, so its model usage is not misattributed to a Graph node. Independent layout batches use bounded concurrency inside one Python Worker process (`MODEL_TRANSLATION_CONCURRENCY=2` by default, valid range `1-4`) and are merged in source order. In-flight calls reserve disjoint shares of the run budget before dispatch, so concurrency cannot multiply the configured ceiling. The Skill uses the same fixed Mini model and OpenRouter privacy/routing policy, reserves cost at `$0.30/M` prompt and `$2.50/M` completion ceilings, persists returned usage plus aggregate provider latency, wall latency and concurrency in the Skill output, and rejects reported cost above the per-run `60,000` micro-USD ceiling or the per-call reservation. Production configuration requires `companion,life,work` in `AGENT_CHAT_MODULES`; the legacy direct-chat model path remains available only for development and staged canaries. An OpenRouter workspace/key budget is still the final hard spending limit.
...
or dense text columns stay top-aligned and use width-aware font sizing and
paragraph spacing; a rendered-line capacity estimate fails the artifact gate
before wrapped body text can escape its placeholder. Narrative lists write an
explicit native bullet character with stable left and hanging indents. Wrapped
CJK list items use a paired native-marker and editable-text layout so renderers
cannot move the marker onto an otherwise empty line while the item text starts
below it. This keeps the marker and first line on the same baseline so the
layout remains stable across renderers. Source-page markers and
code-like identifiers are kept on one rendered line.
...
Async Skill waits use a two-level recovery contract. Success, failure and cancellation write a terminal outbox event in the same transaction as the Skill state; the database-first Agent reconciler advances matching `waiting_tool` runs and appends a durable resume event. In optional Kafka scale mode, terminal events provide immediate wake hints and duplicates are coalesced by run ID, with at most one trailing replay retained when a newer hint arrives during execution. The interrupt still requests an initial 500ms poll with a 5s maximum: the Go Worker checks PostgreSQL first and exponentially reschedules a pending task without launching Python, so missed hints and completion/suspension races remain recoverable. Only a terminal Skill state resumes LangGraph. A resumed checkpoint also reuses its original trusted tool catalog instead of fetching the catalog again. `make eval-agent-runtime` emits the concurrency, backpressure and resume-contract test stream to `artifacts/agent-eval/runtime.jsonl`.
