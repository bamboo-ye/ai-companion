...
用户直接在对话中提出记账和创建提醒，Agent 将自然语言转换为受控工具调用，在确认后写入生活账本。提醒任务首次未完成时，系统保留原对话并提供重试入口；重试成功后返回明确的执行结果，展示了人工确认、失败可恢复和业务结果反馈。

![生活助手对话：记账、提醒创建与失败重试](docs/images/readme/life-conversation.jpg)
...
用户只需描述“生成 7 页项目汇报大纲”的目标和结构要求，系统即可识别任务意图并路由到 PPTX 大纲 Skill。对话中返回任务执行状态，任务历史则持续保存结构化大纲、`receive → validate → plan → execute → deliver` 阶段轨迹、质量门禁与来源覆盖信息，形成可追踪、可重试的异步工作流。
