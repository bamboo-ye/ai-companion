...
from .direct_traffic_certification import (
    PRODUCTION_EXPANSION_100_KEY_ENV as CERTIFICATION_KEY_ENV,
    PRODUCTION_EXPANSION_100_KEY_ID_ENV as CERTIFICATION_KEY_ID_ENV,
...
GATE_SCHEMA_VERSION = "agent-direct-traffic-production-expansion-100-gate-v1"
ATTESTATION_SCHEMA_VERSION = (
    "agent-direct-traffic-production-expansion-100-gate-attestation-v1"
)
GATE_FILE = "direct-traffic-production-expansion-100-gate.json"
ATTESTATION_FILE = "direct-traffic-production-expansion-100-gate-attestation.json"
KEY_ENV = "OBSERVABILITY_DIRECT_TRAFFIC_PRODUCTION_EXPANSION_100_GATE_KEY"
KEY_ID_ENV = "OBSERVABILITY_DIRECT_TRAFFIC_PRODUCTION_EXPANSION_100_GATE_KEY_ID"
SIGNING_DOMAIN = b"ai-companion/direct-traffic-production-expansion-100-gate/v1\0"
MAX_ATTESTATION_SECONDS = 15 * 60
DEFAULT_MINIMUM_HEALTH_WINDOW_SECONDS = 10 * 60
_DIGEST_RE = re.compile(r"[0-9a-f]{64}")
_GATE_ID_RE = re.compile(r"direct-traffic-production-expansion-100-gate-[0-9a-f]{32}")
_ATTESTATION_ID_RE = re.compile(
    r"direct-traffic-production-expansion-100-gate-attestation-[0-9a-f]{32}"
)
_BOUNDARY = ProductionExpansionBoundary(
    ...
    stage_violation="expansion_stage_not_fifty_to_one_hundred",
)


class DirectTrafficProductionExpansion100GateError(ValueError):
    pass


class DirectTrafficProductionExpansion100GateConfigError(
    DirectTrafficProductionExpansion100GateError
):
    pass


class DirectTrafficProductionExpansion100GateVerificationError(
    DirectTrafficProductionExpansion100GateError
):
    pass


def evaluate_expansion_100_gate(
    ...
    *,
    ...
    if len({rollout_key, certification_key, shadow_key}) != 3:
        raise DirectTrafficProductionExpansion100GateConfigError(
    ...
    try:
        ...
        raise DirectTrafficProductionExpansion100GateVerificationError(
    ...
    try:
        ...
        raise DirectTrafficProductionExpansion100GateVerificationError(str(exc)) from exc
    ...
    result["gate_id"] = (
        "direct-traffic-production-expansion-100-gate-"
        f"{_digest(_canonical_json(seed))[:32]}"
    )
    return _validate_gate(result)


def write_expansion_100_gate(output_dir: Path, report: Mapping[str, Any]) -> Path:
...
def attest_expansion_100_gate(
    ...
    *,
    ...
    recomputed = evaluate_expansion_100_gate(
        rollout_attestation,
        rollout_report,
    ...
    payload["attestation_id"] = (
        "direct-traffic-production-expansion-100-gate-attestation-"
    ...
    try:
        ...
        return verify_expansion_100_gate_attestation(
            gate_dir, signing_key, signing_key_id, now=current_time
        )
    return result


def verify_expansion_100_gate_attestation(
    gate_dir: Path,
    key: bytes,
    key_id: str,
    *,
...
def verify_embedded_expansion_100_gate_attestation(
    value: Any,
    key: bytes,
    key_id: str,
    *,
...
def validate_prior_expansion_receipt(value: Any) -> dict[str, Any]:
    ...
    if any(
        (
            ...
            != "agent-direct-traffic-production-expansion-50-receipt-v1",
            result["current_traffic_percent"] != 50,
            result["revision"] != 6,
...
def _validate_gate(value: Any) -> dict[str, Any]:
    ...
    if any(
        (
            ...
            result["current_traffic_percent"] != 50,
            result["target_traffic_percent"] != 100,
        )
    ):
        raise DirectTrafficProductionExpansion100GateVerificationError(
...
def _validate_digest(value: Any, label: str) -> str:
    if not isinstance(value, str) or _DIGEST_RE.fullmatch(value) is None:
        raise DirectTrafficProductionExpansion100GateConfigError(f"{label} is invalid")
    return value
...
def _invalid_json() -> dict[str, Any]:
    raise DirectTrafficProductionExpansion100GateConfigError("JSON contract is invalid")
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Authorize the production 50%-to-100% expansion stage"
    )
    ...
    try:
        ...
        if args.command == "evaluate":
            report = evaluate_expansion_100_gate(
            ...
            write_expansion_100_gate(args.output_dir, report)
        ...
        if args.command == "attest":
            value = attest_expansion_100_gate(
                args.gate_dir,
                gate_key,
                gate_key_id,
            ...
            value = verify_expansion_100_gate_attestation(
                args.gate_dir,
                gate_key,
                gate_key_id,
                max_age_seconds=args.max_age_seconds,
