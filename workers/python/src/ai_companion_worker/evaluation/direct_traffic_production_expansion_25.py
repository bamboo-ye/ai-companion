...
from .direct_rollout import (
    PRODUCTION_EXPANSION_25_KEY_ENV as ROLLOUT_KEY_ENV,
    PRODUCTION_EXPANSION_25_KEY_ID_ENV as ROLLOUT_KEY_ID_ENV,
...
from .direct_traffic_production_expansion_25_gate import (
    KEY_ENV as EXPANSION_25_GATE_KEY_ENV,
    KEY_ID_ENV as EXPANSION_25_GATE_KEY_ID_ENV,
    _read_json,
    _read_gate,
    validate_prior_expansion_receipt,
    verify_expansion_25_gate_attestation,
)
...
from .direct_traffic_shadow_gate import (
    PRODUCTION_EXPANSION_25_KEY_ENV as SHADOW_KEY_ENV,
    PRODUCTION_EXPANSION_25_KEY_ID_ENV as SHADOW_KEY_ID_ENV,
...
ADAPTER_NAME = "https-direct-traffic-production-expansion-25"
IMPLEMENTATION_VERSION = "1.0.0"
EXECUTION_SCHEMA_VERSION = "agent-direct-traffic-production-expansion-25-execution-v1"
STATE_SCHEMA_VERSION = "agent-direct-traffic-production-expansion-25-state-v1"
RESULT_SCHEMA_VERSION = "agent-direct-traffic-production-expansion-25-result-v1"
RECEIPT_SCHEMA_VERSION = "agent-direct-traffic-production-expansion-25-receipt-v1"
TOKEN_ENV = "OBSERVABILITY_DIRECT_TRAFFIC_PRODUCTION_EXPANSION_25_TOKEN"


@dataclass(frozen=True)
class VerifiedProductionExpansion25Execution:
    ...
    expansion_25_gate_attestation: Mapping[str, Any]
    expansion_25_gate_attestation_sha256: str
...
class HTTPSProductionExpansion25DirectTrafficAdapter(HTTPSProductionDirectTrafficAdapter):
    name = ADAPTER_NAME
    ...
    def __init__(self, config: HTTPSPreproductionConfig, **kwargs: Any) -> None:
        prior_tokens = {
            value for name in (
                PRODUCTION_TOKEN_ENV, EMERGENCY_TOKEN_ENV, RECOVERY_TOKEN_ENV,
    ...
    def _validate_authorization(
        ...
        if not isinstance(authorization, VerifiedProductionExpansion25Execution):
            raise DirectTrafficPreproductionConfigError(
                "invalid production expansion authorization"
            )
        return _validate_expansion_25_execution(authorization, self)


def verify_production_expansion_25_execution(
    adapter: HTTPSProductionExpansion25DirectTrafficAdapter,
    ...
    shadow_key: bytes,
    shadow_key_id: str,
    expansion_25_gate_dir: Path,
    expansion_25_gate_key: bytes,
    expansion_25_gate_key_id: str,
    prior_expansion_receipt: Mapping[str, Any],
    ...
    *,
...
) -> VerifiedProductionExpansion25Execution:
    current_time = _normalize_now(now)
    _validate_expansion_25_secrets(
        adapter.config.token, rollout_key, certification_key, shadow_key,
    ...
    gate = verify_expansion_25_gate_attestation(
        expansion_25_gate_dir, expansion_25_gate_key, expansion_25_gate_key_id,
        max_age_seconds=max_age_seconds, now=current_time,
    )
    gate_report = _read_gate(expansion_25_gate_dir, expect_attestation=True)
    receipt = validate_prior_expansion_receipt(prior_expansion_receipt)
    local_state = sandbox.status()
    if any((
        request.decision != "expand",
        request.expected_traffic_percent != 10,
        request.target_traffic_percent != 25,
        request.expected_revision != 4,
        local_state["current_traffic_percent"] != 10,
        local_state["revision"] != 4,
        local_state["operations"] != 4,
        receipt["history_chain_sha256"] != local_state["history_chain_sha256"],
        receipt["namespace_sha256"] != adapter.namespace_sha256,
    )):
        raise DirectTrafficPreproductionConfigError(
            "expansion permits only the proven 10%-to-25% transition"
        )
    digests = {
    ...
    payload: dict[str, Any] = {
        ...
        "expansion_25_gate_attestation": dict(gate),
        "expansion_25_gate_attestation_sha256": gate_sha256,
    ...
    return _validate_expansion_25_execution(
        VerifiedProductionExpansion25Execution(
            ...
            receipt, digests["prior_expansion_receipt_sha256"], local_state, payload,
        ),
        adapter,
    )


def apply_verified_production_expansion_25_execution(
    adapter: HTTPSProductionExpansion25DirectTrafficAdapter,
    sandbox: SQLiteDirectTrafficSandbox,
    release_history_path: Path,
    authorization: VerifiedProductionExpansion25Execution,
) -> dict[str, Any]:
    validated = _validate_expansion_25_execution(authorization, adapter)
    ...
    return {
        ...
        "expansion_25_gate_attestation_id": (
            validated.expansion_25_gate_attestation["attestation_id"]
        ),
...
def _validate_expansion_25_execution(
    value: VerifiedProductionExpansion25Execution,
    adapter: HTTPSProductionExpansion25DirectTrafficAdapter,
) -> VerifiedProductionExpansion25Execution:
    fields = {
        ...
        "shadow_attestation_sha256", "expansion_25_gate_attestation",
        "expansion_25_gate_attestation_sha256", "prior_expansion_receipt",
        "prior_expansion_receipt_sha256", "expected_state", "request_sha256",
    ...
    checks = (
        ...
        payload["expansion_25_gate_attestation"] == dict(value.expansion_25_gate_attestation),
        payload["expansion_25_gate_attestation_sha256"]
        == value.expansion_25_gate_attestation_sha256,
        ...
        == value.expansion_25_gate_attestation.get("rollout_report_sha256"),
        payload["shadow_gate_report_sha256"]
        == value.expansion_25_gate_attestation.get("shadow_gate_report_sha256"),
        ...
        value.expansion_25_gate_attestation_sha256
        == _digest(_canonical_json(dict(value.expansion_25_gate_attestation))),
        value.prior_expansion_receipt_sha256
...
def _validate_expansion_25_secrets(token: str, *keys: bytes) -> None:
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Apply the production 10%-to-25% expansion"
    )
    ...
    parser.add_argument(
        "--canary-lease-root", type=Path, default=DEFAULT_CANARY_LEASE_ROOT
    )
    ...
    try:
        ...
        certification_key, certification_key_id = certification_key_from_environment(
            profile="production_expansion_25"
        )
        shadow_key, shadow_key_id = _key_from_environment(
            SHADOW_KEY_ENV, SHADOW_KEY_ID_ENV
        )
        gate_key, gate_key_id = _key_from_environment(
            EXPANSION_25_GATE_KEY_ENV, EXPANSION_25_GATE_KEY_ID_ENV
        )
        authorization = verify_production_expansion_25_execution(
            ...
            shadow_history, shadow_key, shadow_key_id, args.expansion_25_gate_dir,
            gate_key, gate_key_id, _read_json(args.prior_expansion_receipt),
            _read_json(args.decision_dir / ROLLOUT_REPORT_FILE),
            _read_json(
                args.shadow_gate_dir / "direct-traffic-shadow-gate-report.json"
            ),
            max_age_seconds=args.max_age_seconds,
        )
        receipt = apply_verified_production_expansion_25_execution(
