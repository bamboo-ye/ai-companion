...
from .direct_traffic_production_expansion_25 import (
    HTTPSProductionExpansion25DirectTrafficAdapter,
    TOKEN_ENV as EXPANSION_25_TOKEN_ENV,
)
from .direct_traffic_production_expansion_50_gate import (
    KEY_ENV as EXPANSION_50_GATE_KEY_ENV,
    KEY_ID_ENV as EXPANSION_50_GATE_KEY_ID_ENV,
    _read_gate,
    _read_json,
    validate_prior_expansion_receipt,
    verify_expansion_50_gate_attestation,
)
...
EXECUTION_SCHEMA_VERSION = "agent-direct-traffic-production-expansion-50-execution-v1"
STATE_SCHEMA_VERSION = "agent-direct-traffic-production-expansion-50-state-v1"
RESULT_SCHEMA_VERSION = "agent-direct-traffic-production-expansion-50-result-v1"
RECEIPT_SCHEMA_VERSION = "agent-direct-traffic-production-expansion-50-receipt-v1"
TOKEN_ENV = "OBSERVABILITY_DIRECT_TRAFFIC_PRODUCTION_EXPANSION_50_TOKEN"


@dataclass(frozen=True)
class VerifiedProductionExpansion50Execution:
    ...
    expansion_50_gate_attestation: Mapping[str, Any]
    expansion_50_gate_attestation_sha256: str
...
class HTTPSProductionExpansion50DirectTrafficAdapter(
    HTTPSProductionExpansion25DirectTrafficAdapter
):
    name = ADAPTER_NAME
    ...
    def _validate_authorization(
        ...
        if not isinstance(authorization, VerifiedProductionExpansion50Execution):
            raise DirectTrafficPreproductionConfigError(
                "invalid production expansion authorization"
            )
        return _validate_expansion_50_execution(authorization, self)


def verify_production_expansion_50_execution(
    adapter: HTTPSProductionExpansion50DirectTrafficAdapter,
    ...
    shadow_key: bytes,
    shadow_key_id: str,
    expansion_50_gate_dir: Path,
    expansion_50_gate_key: bytes,
    expansion_50_gate_key_id: str,
    prior_expansion_receipt: Mapping[str, Any],
    ...
    *,
...
) -> VerifiedProductionExpansion50Execution:
    current_time = _normalize_now(now)
    _validate_expansion_50_secrets(
    ...
    gate = verify_expansion_50_gate_attestation(
    ...
    gate_report = _read_gate(expansion_50_gate_dir, expect_attestation=True)
    receipt = validate_prior_expansion_receipt(prior_expansion_receipt)
    local_state = sandbox.status()
    if any(
        (
            request.decision != "expand",
            request.expected_traffic_percent != 25,
            request.target_traffic_percent != 50,
            request.expected_revision != 5,
            local_state["current_traffic_percent"] != 25,
            local_state["revision"] != 5,
            local_state["operations"] != 5,
            receipt["history_chain_sha256"] != local_state["history_chain_sha256"],
            receipt["namespace_sha256"] != adapter.namespace_sha256,
        )
    ):
        raise DirectTrafficPreproductionConfigError(
            "expansion permits only the proven 25%-to-50% transition"
        )
    digests = {
    ...
    payload: dict[str, Any] = {
        ...
        "expansion_50_gate_attestation": dict(gate),
        "expansion_50_gate_attestation_sha256": gate_sha256,
    ...
    return _validate_expansion_50_execution(
        VerifiedProductionExpansion50Execution(
            request,
            rollout,
            certification,
...
def apply_verified_production_expansion_50_execution(
    adapter: HTTPSProductionExpansion50DirectTrafficAdapter,
    sandbox: SQLiteDirectTrafficSandbox,
    release_history_path: Path,
    authorization: VerifiedProductionExpansion50Execution,
) -> dict[str, Any]:
    validated = _validate_expansion_50_execution(authorization, adapter)
    ...
    return {
        ...
        "expansion_50_gate_attestation_id": (
            validated.expansion_50_gate_attestation["attestation_id"]
        ),
...
def _validate_expansion_50_execution(
    value: VerifiedProductionExpansion50Execution,
    adapter: HTTPSProductionExpansion50DirectTrafficAdapter,
) -> VerifiedProductionExpansion50Execution:
    fields = {
        "schema_version",
        ...
        "expansion_50_gate_attestation",
        "expansion_50_gate_attestation_sha256",
    ...
    checks = (
        ...
        payload["expansion_50_gate_attestation"]
        == dict(value.expansion_50_gate_attestation),
        payload["expansion_50_gate_attestation_sha256"]
        == value.expansion_50_gate_attestation_sha256,
        ...
        == value.expansion_50_gate_attestation.get("rollout_report_sha256"),
        payload["shadow_gate_report_sha256"]
        == value.expansion_50_gate_attestation.get("shadow_gate_report_sha256"),
        ...
        value.expansion_50_gate_attestation_sha256
        == _digest(_canonical_json(dict(value.expansion_50_gate_attestation))),
        value.prior_expansion_receipt_sha256
...
def _validate_expansion_50_secrets(token: str, *keys: bytes) -> None:
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Apply the production 25%-to-50% expansion")
    ...
    try:
        ...
        certification_key, certification_key_id = certification_key_from_environment(
            profile="production_expansion_50"
        )
        shadow_key, shadow_key_id = _key_from_environment(
            SHADOW_KEY_ENV, SHADOW_KEY_ID_ENV
        )
        gate_key, gate_key_id = _key_from_environment(
            EXPANSION_50_GATE_KEY_ENV, EXPANSION_50_GATE_KEY_ID_ENV
        )
        authorization = verify_production_expansion_50_execution(
            adapter,
            sandbox,
            args.decision_dir,
            history,
        ...
        receipt = apply_verified_production_expansion_50_execution(
