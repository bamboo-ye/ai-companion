...
from .direct_traffic_production_expansion_50_gate import (
    ...
    verify_embedded_expansion_50_gate_attestation,
)
...
class SQLiteProductionExpansion50CertificationAdapter(
...
class SQLiteProductionExpansion50Platform(SQLiteProductionExpansion25Platform):
    ...
    def __init__(
        ...
        *,
        ...
        self._expansion_50_gate_key = expansion_50_gate_key
        self._expansion_50_gate_key_id = expansion_50_gate_key_id
        super().__init__(
            path,
            environment_sha256,
            provider_instance,
    ...
    def _validate_execution_payload(
        ...
        try:
            ...
            gate = dict(result["expansion_50_gate_attestation"])
        ...
        if any(
            (
                ...
                result["expansion_50_gate_attestation_sha256"]
                ...
                expected_state.get("current_traffic_percent") != 25,
                expected_state.get("revision") != 5,
                expected_state.get("operations") != 5,
    ...
    def _verify_expansion_proofs(
        ...
        if any(
            (
                ...
                rollout.get("current_traffic_percent") != 25,
                rollout.get("target_traffic_percent") != 50,
        ...
        try:
            verify_embedded_expansion_50_gate_attestation(
                gate,
                self._expansion_50_gate_key,
                self._expansion_50_gate_key_id,
        ...
        except DirectTrafficProductionExpansion50GateError as exc:
            raise PreproductionProviderError("provider_expansion_50_gate_proof") from exc


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Certify the isolated production 25%-to-50% expansion writer"
    )
    ...
    try:
        adapter = SQLiteProductionExpansion50CertificationAdapter(
