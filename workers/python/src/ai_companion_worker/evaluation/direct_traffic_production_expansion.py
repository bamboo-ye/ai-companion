...
from .direct_traffic_production_expansion_gate import (
    KEY_ENV as EXPANSION_GATE_KEY_ENV,
    KEY_ID_ENV as EXPANSION_GATE_KEY_ID_ENV,
    _read_json,
    _read_gate,
    validate_recovery_receipt,
    verify_expansion_gate_attestation,
)
...
ADAPTER_NAME = "https-direct-traffic-production-expansion-10"
IMPLEMENTATION_VERSION = "1.0.0"
EXECUTION_SCHEMA_VERSION = "agent-direct-traffic-production-expansion-10-execution-v1"
STATE_SCHEMA_VERSION = "agent-direct-traffic-production-expansion-10-state-v1"
RESULT_SCHEMA_VERSION = "agent-direct-traffic-production-expansion-10-result-v1"
RECEIPT_SCHEMA_VERSION = "agent-direct-traffic-production-expansion-10-receipt-v1"
TOKEN_ENV = "OBSERVABILITY_DIRECT_TRAFFIC_PRODUCTION_EXPANSION_TOKEN"


@dataclass(frozen=True)
class VerifiedProductionExpansionExecution:
    ...
    expansion_gate_attestation: Mapping[str, Any]
    expansion_gate_attestation_sha256: str
...
class HTTPSProductionExpansionDirectTrafficAdapter(HTTPSProductionDirectTrafficAdapter):
    name = ADAPTER_NAME
    ...
    def __init__(self, config: HTTPSPreproductionConfig, **kwargs: Any) -> None:
        prior_tokens = {
            value for name in (PRODUCTION_TOKEN_ENV, EMERGENCY_TOKEN_ENV, RECOVERY_TOKEN_ENV)
    ...
    def _validate_authorization(
        ...
        if not isinstance(authorization, VerifiedProductionExpansionExecution):
            raise DirectTrafficPreproductionConfigError(
                "invalid production expansion authorization"
            )
        return _validate_expansion_execution(authorization, self)


def verify_production_expansion_execution(
    adapter: HTTPSProductionExpansionDirectTrafficAdapter,
    ...
    shadow_key: bytes,
    shadow_key_id: str,
    expansion_gate_dir: Path,
    expansion_gate_key: bytes,
    expansion_gate_key_id: str,
    recovery_receipt: Mapping[str, Any],
    ...
    *,
...
) -> VerifiedProductionExpansionExecution:
    current_time = _normalize_now(now)
    _validate_expansion_secrets(
    ...
    gate = verify_expansion_gate_attestation(
        expansion_gate_dir, expansion_gate_key, expansion_gate_key_id,
        max_age_seconds=max_age_seconds, now=current_time,
    )
    gate_report = _read_gate(expansion_gate_dir, expect_attestation=True)
    receipt = validate_recovery_receipt(recovery_receipt)
    local_state = sandbox.status()
    if any((
        request.decision != "expand",
        request.expected_traffic_percent != 5,
        request.target_traffic_percent != 10,
        request.expected_revision != 3,
        local_state["current_traffic_percent"] != 5,
        local_state["revision"] != 3,
        local_state["operations"] != 3,
        receipt["history_chain_sha256"] != local_state["history_chain_sha256"],
        receipt["namespace_sha256"] != adapter.namespace_sha256,
    )):
        raise DirectTrafficPreproductionConfigError(
            "expansion permits only the recovered 5%-to-10% transition"
        )
    digests = {
    ...
    payload: dict[str, Any] = {
        ...
        "expansion_gate_attestation": dict(gate),
        "expansion_gate_attestation_sha256": gate_sha256,
        "recovery_receipt": receipt,
        "recovery_receipt_sha256": digests["recovery_receipt_sha256"],
    ...
    return _validate_expansion_execution(
        VerifiedProductionExpansionExecution(
            ...
            receipt, digests["recovery_receipt_sha256"], local_state, payload,
        ),
        adapter,
    )


def apply_verified_production_expansion_execution(
    adapter: HTTPSProductionExpansionDirectTrafficAdapter,
    sandbox: SQLiteDirectTrafficSandbox,
    release_history_path: Path,
    authorization: VerifiedProductionExpansionExecution,
) -> dict[str, Any]:
    validated = _validate_expansion_execution(authorization, adapter)
    ...
    return {
        ...
        "expansion_gate_attestation_id": (
            validated.expansion_gate_attestation["attestation_id"]
        ),
        "recovery_operation_id": validated.recovery_receipt["operation_id"],
...
def _validate_expansion_execution(
    value: VerifiedProductionExpansionExecution,
    adapter: HTTPSProductionExpansionDirectTrafficAdapter,
) -> VerifiedProductionExpansionExecution:
    fields = {
        ...
        "shadow_attestation_sha256", "expansion_gate_attestation",
        "expansion_gate_attestation_sha256", "recovery_receipt",
        "recovery_receipt_sha256", "expected_state", "request_sha256",
    ...
    checks = (
        ...
        payload["expansion_gate_attestation"] == dict(value.expansion_gate_attestation),
        payload["expansion_gate_attestation_sha256"]
        == value.expansion_gate_attestation_sha256,
        payload["recovery_receipt"] == dict(value.recovery_receipt),
        payload["recovery_receipt_sha256"] == value.recovery_receipt_sha256,
        payload["rollout_report_sha256"]
        == value.expansion_gate_attestation.get("rollout_report_sha256"),
        payload["shadow_gate_report_sha256"]
        == value.expansion_gate_attestation.get("shadow_gate_report_sha256"),
        ...
        value.expansion_gate_attestation_sha256
        == _digest(_canonical_json(dict(value.expansion_gate_attestation))),
        value.recovery_receipt_sha256
        == _digest(_canonical_json(dict(value.recovery_receipt))),
...
def _validate_expansion_secrets(token: str, *keys: bytes) -> None:
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Apply the production 5%-to-10% expansion"
    )
    ...
    try:
        ...
        certification_key, certification_key_id = certification_key_from_environment(
            profile="production_expansion"
        )
        shadow_key, shadow_key_id = _key_from_environment(
            SHADOW_KEY_ENV, SHADOW_KEY_ID_ENV
        )
        gate_key, gate_key_id = _key_from_environment(
            EXPANSION_GATE_KEY_ENV, EXPANSION_GATE_KEY_ID_ENV
        )
        authorization = verify_production_expansion_execution(
            ...
            shadow_history, shadow_key, shadow_key_id, args.expansion_gate_dir,
            gate_key, gate_key_id, _read_json(args.recovery_receipt),
        ...
        receipt = apply_verified_production_expansion_execution(
