...
class SQLiteProductionExpansion25Platform(SQLitePreproductionPlatform):
    ...
    def __init__(
        ...
        *,
        ...
        if len({rollout_key, certification_key, shadow_key, expansion_25_gate_key}) != 4:
            raise PreproductionProviderError("provider_expansion_proof_key_reuse")
        self._expansion_25_gate_key = expansion_25_gate_key
        self._expansion_25_gate_key_id = expansion_25_gate_key_id
        super().__init__(
    ...
    def _validate_execution_payload(
        ...
        try:
            ...
            gate = dict(result["expansion_25_gate_attestation"])
        ...
        if any((
            ...
            result["expansion_25_gate_attestation_sha256"] != _digest(_canonical_json(gate)),
            result["prior_expansion_receipt_sha256"] != _digest(_canonical_json(prior)),
            ...
            expected_state.get("current_traffic_percent") != 10,
            expected_state.get("revision") != 4,
            expected_state.get("operations") != 4,
        ...
        self._verify_expansion_proofs(
            rollout, certification, shadow, gate, prior, request,
    ...
    def _verify_expansion_proofs(
        ...
        if any((
            ...
            rollout.get("current_traffic_percent") != 10,
            rollout.get("target_traffic_percent") != 25,
        ...
        try:
            verify_embedded_expansion_25_gate_attestation(
                gate, self._expansion_25_gate_key, self._expansion_25_gate_key_id,
        ...
        except DirectTrafficProductionExpansion25GateError as exc:
            raise PreproductionProviderError("provider_expansion_25_gate_proof") from exc


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Certify the isolated production 10%-to-25% expansion writer"
    )
    ...
    try:
        adapter = SQLiteProductionExpansion25CertificationAdapter(
        ...
        key, key_id = certification_key_from_environment(profile="production_expansion_25")
