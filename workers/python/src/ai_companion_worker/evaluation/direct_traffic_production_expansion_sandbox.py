...
from .direct_traffic_production_expansion import (
    ADAPTER_NAME,
...
class SQLiteProductionExpansionPlatform(SQLitePreproductionPlatform):
    ...
    def __init__(
        ...
        *,
        ...
        if len({rollout_key, certification_key, shadow_key, expansion_gate_key}) != 4:
            raise PreproductionProviderError("provider_expansion_proof_key_reuse")
        self._expansion_gate_key = expansion_gate_key
        self._expansion_gate_key_id = expansion_gate_key_id
        super().__init__(
    ...
    def _validate_execution_payload(
        ...
        fields = {
            "schema_version", "namespace_sha256", "rollout_request",
            "rollout_attestation", "adapter_certification",
            "adapter_certification_sha256", "shadow_attestation",
            ...
            "rollout_report_sha256", "shadow_gate_report_sha256",
        }
        ...
        try:
            ...
            gate = dict(result["expansion_gate_attestation"])
            recovery = validate_recovery_receipt(result["recovery_receipt"])
        ...
        if any((
            ...
            result["expansion_gate_attestation_sha256"] != _digest(_canonical_json(gate)),
            result["recovery_receipt_sha256"] != _digest(_canonical_json(recovery)),
            certification.get("adapter") != self.adapter_name,
            certification.get("implementation_version") != self.implementation_version,
            certification.get("provider_instance_sha256") != self.provider_instance_sha256,
            certification.get("environment_sha256") != self.environment_sha256,
            shadow.get("provider_instance_sha256") != self.provider_instance_sha256,
            shadow.get("environment_sha256") != self.environment_sha256,
            expected_state.get("current_traffic_percent") != 5,
            expected_state.get("revision") != 3,
            expected_state.get("operations") != 3,
            recovery.get("namespace_sha256") != self.namespace_sha256,
            recovery.get("history_chain_sha256")
        ...
        self._verify_expansion_proofs(
            rollout, certification, shadow, gate, recovery, request,
    ...
    def _verify_expansion_proofs(
        ...
        try:
            verify_embedded_expansion_gate_attestation(
                gate, self._expansion_gate_key, self._expansion_gate_key_id,
                ...
                recovery_receipt_sha256=_digest(_canonical_json(recovery)),
                now=now,
            )
        except DirectTrafficProductionExpansionGateError as exc:
            raise PreproductionProviderError("provider_expansion_gate_proof") from exc


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Certify the isolated production 5%-to-10% expansion writer"
    )
    ...
    try:
        adapter = SQLiteProductionExpansionCertificationAdapter(
        ...
        key, key_id = certification_key_from_environment(profile="production_expansion")
