...
from .direct_rollout import (
    PRODUCTION_EXPANSION_50_KEY_ENV as ROLLOUT_KEY_ENV,
    PRODUCTION_EXPANSION_50_KEY_ID_ENV as ROLLOUT_KEY_ID_ENV,
    DirectRolloutError,
...
from .direct_traffic_certification import (
    PRODUCTION_EXPANSION_50_KEY_ENV as CERTIFICATION_KEY_ENV,
    PRODUCTION_EXPANSION_50_KEY_ID_ENV as CERTIFICATION_KEY_ID_ENV,
...
from .direct_traffic_shadow_gate import (
    PRODUCTION_EXPANSION_50_KEY_ENV as SHADOW_KEY_ENV,
    PRODUCTION_EXPANSION_50_KEY_ID_ENV as SHADOW_KEY_ID_ENV,
...
GATE_SCHEMA_VERSION = "agent-direct-traffic-production-expansion-50-gate-v1"
ATTESTATION_SCHEMA_VERSION = (
    "agent-direct-traffic-production-expansion-50-gate-attestation-v1"
)
GATE_FILE = "direct-traffic-production-expansion-50-gate.json"
ATTESTATION_FILE = "direct-traffic-production-expansion-50-gate-attestation.json"
KEY_ENV = "OBSERVABILITY_DIRECT_TRAFFIC_PRODUCTION_EXPANSION_50_GATE_KEY"
KEY_ID_ENV = "OBSERVABILITY_DIRECT_TRAFFIC_PRODUCTION_EXPANSION_50_GATE_KEY_ID"
SIGNING_DOMAIN = b"ai-companion/direct-traffic-production-expansion-50-gate/v1\0"
MAX_ATTESTATION_SECONDS = 15 * 60
DEFAULT_MINIMUM_HEALTH_WINDOW_SECONDS = 10 * 60
_DIGEST_RE = re.compile(r"[0-9a-f]{64}")
_GATE_ID_RE = re.compile(r"direct-traffic-production-expansion-50-gate-[0-9a-f]{32}")
_ATTESTATION_ID_RE = re.compile(
    r"direct-traffic-production-expansion-50-gate-attestation-[0-9a-f]{32}"
)
_BOUNDARY = ProductionExpansionBoundary(
    current_traffic_percent=25,
    target_traffic_percent=50,
    adapter_name="https-direct-traffic-production-expansion-50",
    adapter_version="1.0.0",
    stage_violation="expansion_stage_not_twenty_five_to_fifty",
)


class DirectTrafficProductionExpansion50GateError(ValueError):
    pass


class DirectTrafficProductionExpansion50GateConfigError(
    DirectTrafficProductionExpansion50GateError
):
    pass


class DirectTrafficProductionExpansion50GateVerificationError(
    DirectTrafficProductionExpansion50GateError
):
    pass


def evaluate_expansion_50_gate(
    ...
    *,
    ...
    if len({rollout_key, certification_key, shadow_key}) != 3:
        raise DirectTrafficProductionExpansion50GateConfigError(
    ...
    try:
        ...
        raise DirectTrafficProductionExpansion50GateVerificationError(
    ...
    try:
        ...
        raise DirectTrafficProductionExpansion50GateVerificationError(str(exc)) from exc
    ...
    result["gate_id"] = (
        "direct-traffic-production-expansion-50-gate-"
        f"{_digest(_canonical_json(seed))[:32]}"
    )
    return _validate_gate(result)


def write_expansion_50_gate(output_dir: Path, report: Mapping[str, Any]) -> Path:
...
def attest_expansion_50_gate(
    ...
    *,
    ...
    recomputed = evaluate_expansion_50_gate(
        rollout_attestation,
        rollout_report,
    ...
    payload["attestation_id"] = (
        "direct-traffic-production-expansion-50-gate-attestation-"
    ...
    try:
        ...
        return verify_expansion_50_gate_attestation(
            gate_dir, signing_key, signing_key_id, now=current_time
        )
    return result


def verify_expansion_50_gate_attestation(
    gate_dir: Path,
    key: bytes,
    key_id: str,
    *,
...
def verify_embedded_expansion_50_gate_attestation(
    value: Any,
    key: bytes,
    key_id: str,
    *,
...
def validate_prior_expansion_receipt(value: Any) -> dict[str, Any]:
    ...
    if any(
        (
            ...
            != "agent-direct-traffic-production-expansion-25-receipt-v1",
            result["current_traffic_percent"] != 25,
            result["revision"] != 5,
...
def _validate_gate(value: Any) -> dict[str, Any]:
    ...
    if any(
        (
            ...
            result["current_traffic_percent"] != 25,
            result["target_traffic_percent"] != 50,
        )
    ):
        raise DirectTrafficProductionExpansion50GateVerificationError(
...
def _validate_digest(value: Any, label: str) -> str:
    if not isinstance(value, str) or _DIGEST_RE.fullmatch(value) is None:
        raise DirectTrafficProductionExpansion50GateConfigError(f"{label} is invalid")
    return value
...
def _invalid_json() -> dict[str, Any]:
    raise DirectTrafficProductionExpansion50GateConfigError("JSON contract is invalid")
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Authorize the production 25%-to-50% expansion stage"
    )
    ...
    try:
        ...
        if args.command == "evaluate":
            report = evaluate_expansion_50_gate(
            ...
            write_expansion_50_gate(args.output_dir, report)
        ...
        if args.command == "attest":
            value = attest_expansion_50_gate(
                args.gate_dir,
                gate_key,
                gate_key_id,
            ...
            value = verify_expansion_50_gate_attestation(
                args.gate_dir,
                gate_key,
                gate_key_id,
                max_age_seconds=args.max_age_seconds,
