...
from .direct_traffic_certification import (
    PRODUCTION_EXPANSION_25_KEY_ENV as CERTIFICATION_KEY_ENV,
    PRODUCTION_EXPANSION_25_KEY_ID_ENV as CERTIFICATION_KEY_ID_ENV,
...
from .direct_traffic_production_stage import (
    ProductionExpansionBoundary,
    ProductionStageEvidenceError,
    evaluate_production_stage_evidence,
)
from .release_attestation import MAX_CLOCK_SKEW_SECONDS


GATE_SCHEMA_VERSION = "agent-direct-traffic-production-expansion-25-gate-v1"
ATTESTATION_SCHEMA_VERSION = (
    "agent-direct-traffic-production-expansion-25-gate-attestation-v1"
)
GATE_FILE = "direct-traffic-production-expansion-25-gate.json"
ATTESTATION_FILE = "direct-traffic-production-expansion-25-gate-attestation.json"
KEY_ENV = "OBSERVABILITY_DIRECT_TRAFFIC_PRODUCTION_EXPANSION_25_GATE_KEY"
KEY_ID_ENV = "OBSERVABILITY_DIRECT_TRAFFIC_PRODUCTION_EXPANSION_25_GATE_KEY_ID"
SIGNING_DOMAIN = b"ai-companion/direct-traffic-production-expansion-25-gate/v1\0"
MAX_ATTESTATION_SECONDS = 15 * 60
DEFAULT_MINIMUM_HEALTH_WINDOW_SECONDS = 10 * 60
_DIGEST_RE = re.compile(r"[0-9a-f]{64}")
_GATE_ID_RE = re.compile(r"direct-traffic-production-expansion-25-gate-[0-9a-f]{32}")
_ATTESTATION_ID_RE = re.compile(
    r"direct-traffic-production-expansion-25-gate-attestation-[0-9a-f]{32}"
)
_BOUNDARY = ProductionExpansionBoundary(
    current_traffic_percent=10,
    target_traffic_percent=25,
    adapter_name="https-direct-traffic-production-expansion-25",
    adapter_version="1.0.0",
    stage_violation="expansion_stage_not_ten_to_twenty_five",
)


class DirectTrafficProductionExpansion25GateError(ValueError):
    pass


class DirectTrafficProductionExpansion25GateConfigError(
    DirectTrafficProductionExpansion25GateError
):
    pass


class DirectTrafficProductionExpansion25GateVerificationError(
    DirectTrafficProductionExpansion25GateError
):
    pass


def evaluate_expansion_25_gate(
    ...
    *,
    ...
    if len({rollout_key, certification_key, shadow_key}) != 3:
        raise DirectTrafficProductionExpansion25GateConfigError(
    ...
    try:
        ...
        raise DirectTrafficProductionExpansion25GateVerificationError(
    ...
    try:
        evidence = evaluate_production_stage_evidence(
            _BOUNDARY,
            rollout,
            report,
            certification,
    ...
    except ProductionStageEvidenceError as exc:
        raise DirectTrafficProductionExpansion25GateVerificationError(str(exc)) from exc
    ...
    report_result: dict[str, Any] = {
        ...
        "decision": "pass" if not evidence.violations else "hold",
        **bindings,
        "current_traffic_percent": 10,
        "target_traffic_percent": 25,
        "health_window_seconds": evidence.health_window_seconds,
    ...
    report_result["gate_id"] = (
        "direct-traffic-production-expansion-25-gate-"
        f"{_digest(_canonical_json(seed))[:32]}"
    )
    return _validate_gate(report_result)


def write_expansion_25_gate(output_dir: Path, report: Mapping[str, Any]) -> Path:
...
def attest_expansion_25_gate(
    ...
    *,
    ...
    recomputed = evaluate_expansion_25_gate(
        rollout_attestation, rollout_report, adapter_certification,
        shadow_attestation, shadow_gate_report, prior_expansion_receipt,
    ...
    payload["attestation_id"] = (
        "direct-traffic-production-expansion-25-gate-attestation-"
    ...
    try:
        ...
        return verify_expansion_25_gate_attestation(
            gate_dir, signing_key, signing_key_id, now=current_time
        )
    return result


def verify_expansion_25_gate_attestation(
    gate_dir: Path,
    key: bytes,
    key_id: str,
    *,
...
def verify_embedded_expansion_25_gate_attestation(
    value: Any,
    key: bytes,
    key_id: str,
    *,
    ...
    expected = {
        ...
        "prior_expansion_receipt_sha256": prior_expansion_receipt_sha256,
        "current_traffic_percent": 10,
        "target_traffic_percent": 25,
        "decision": "pass",
    }
...
def validate_prior_expansion_receipt(value: Any) -> dict[str, Any]:
    fields = {
        ...
        "expansion_gate_attestation_id", "recovery_operation_id",
        "remote_reused", "local_reused", "applied_at",
    }
    if not isinstance(value, Mapping) or set(value) != fields:
        raise DirectTrafficProductionExpansion25GateConfigError(
            "prior expansion receipt contract is invalid"
        )
    result = dict(value)
    if any((
        result["schema_version"]
        != "agent-direct-traffic-production-expansion-10-receipt-v1",
        result["current_traffic_percent"] != 10,
        result["revision"] != 4,
        result["outcome"] not in {"applied", "no_op"},
    )):
        raise DirectTrafficProductionExpansion25GateConfigError(
            "prior expansion receipt state is invalid"
        )
    for field in ("namespace_sha256", "history_chain_sha256"):
        _validate_digest(result[field], field)
    _parse_timestamp(str(result["applied_at"]), "prior expansion applied_at")
    return result
...
def _validate_gate(value: Any) -> dict[str, Any]:
    ...
    if any((
        ...
        result["current_traffic_percent"] != 10,
        result["target_traffic_percent"] != 25,
    )):
        raise DirectTrafficProductionExpansion25GateVerificationError(
...
def _validate_attestation(value: Any) -> dict[str, Any]:
    fields = {
        ...
        "adapter_certification_sha256", "shadow_attestation_sha256",
        "shadow_gate_report_sha256",
        "prior_expansion_receipt_sha256", "key_id", "algorithm", "signature",
    }
    if not isinstance(value, Mapping) or set(value) != fields:
        raise DirectTrafficProductionExpansion25GateVerificationError(
...
def _validate_digest(value: Any, label: str) -> str:
    if not isinstance(value, str) or _DIGEST_RE.fullmatch(value) is None:
        raise DirectTrafficProductionExpansion25GateConfigError(f"{label} is invalid")
    return value


def _validate_age(value: Any) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or not 1 <= value <= 86400:
        raise DirectTrafficProductionExpansion25GateConfigError("expansion age is invalid")
    return value
...
def _invalid_json() -> dict[str, Any]:
    raise DirectTrafficProductionExpansion25GateConfigError("JSON contract is invalid")
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Authorize the production 10%-to-25% expansion stage"
    )
    ...
    for command in (evaluate, attest):
        ...
        command.add_argument("--prior-expansion-receipt", type=Path, required=True)
    ...
    try:
        ...
        if args.command == "evaluate":
            report = evaluate_expansion_25_gate(
            ...
            write_expansion_25_gate(args.output_dir, report)
        ...
        if args.command == "attest":
            value = attest_expansion_25_gate(
                args.gate_dir,
                gate_key,
                gate_key_id,
                ...
                prior_expansion_receipt=_read_json(args.prior_expansion_receipt),
                rollout_key=rollout_key,
                rollout_key_id=rollout_key_id,
                certification_key=certification_key,
                certification_key_id=certification_key_id,
            ...
            value = verify_expansion_25_gate_attestation(
                args.gate_dir,
                gate_key,
                gate_key_id,
                max_age_seconds=args.max_age_seconds,
