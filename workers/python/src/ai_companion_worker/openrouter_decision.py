        ...
        *,
        ...
        if tool_name in _PRESENTATION_TOOLS:
            system_prompt += (
                ...
                "涉及数学公式、算法定义、代码、参数和实验数值时，必须忠实保留当前可信来源"
                "中的系数、分子分母、运算符、变量下标和数值；禁止为了简化而代数改写、重新"
                "缩放或猜测缺失符号。若来源文本不足以确认公式，只写来源明确支持的定性结论，"
                "不得自行补出公式。尤其不得把 PDF 二维排版的 argmin/argmax 公式线性重排；"
                "这类公式只概括用途并指向原文页，由来源截图展示精确公式。"
